    ...
    def draw(self, surface):
        ...
        if hasattr(surface, "blits"):
            self.spritedict.update(
                zip(
                    sprites,
                    surface.blits((spr.image, (spr.rect.x+spr.offset[0],spr.rect.y+spr.offset[1])) for spr in sprites)
                )
            )
        else:
            for spr in sprites:
                # MODIFIED OFFSET 
                self.spritedict[spr] = surface.blit(spr.image, (spr.rect.x+spr.offset[0],spr.rect.y+spr.offset[1]))
        self.lostsprites = []
